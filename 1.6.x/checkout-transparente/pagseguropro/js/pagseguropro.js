...
$(document).ready(function(){
    ...
    parcelasSemJuros = Number($('#parcelasSemJuros').val());
...
function ps_getInstallments(bandeira) {
    ...
    PagSeguroDirectPayment.getInstallments({
		...
		success: function(resposta) {
			...
			if (typeof(max_parcelas) == 'undefined' || max_parcelas < 2) {
			...
			for (var i in parcelamento) {
				...
				if (optionItem.quantity <= max_parcelas) {
					opts += '<option value="' + optionItem.quantity + '" dataPrice="' + valor + '">'+ optionLabel +'</option>';
				}
			};
			$('#card_inst').html(opts);
		},
...
function ps_validarCartao() {
    ...
    if (valor_parc.length == 0) {
    	html += 'Valor das Parcelas não definido. <br>';
		errorFields.push('card_inst');
    }
