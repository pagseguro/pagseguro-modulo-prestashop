...
$(document).ready(function() {
    ...
    tipoParcelaMinima = Number($('#tipo_parcela_minima').val());
    ...
    $('#submitBankSlip').click(function (e) {
        ps_finalizarBoleto();
    });
    $('#submitDebito').click(function (e) {
        ps_finalizarTransf();
    });
    ...
    $("#card_number").blur(function (e) {
        ...
        if (thisNum !== '') {
            if (ps_getBrand(thisNum) === '' || ps_getBrand(thisNum) === false) {
                $("#credit-icon").html('<i class="icon-credit-card"></i>');
            }
        } else {
            $(this).parent().parent().removeClass('form-ok').removeClass('form-error');
            $("#credit-icon").html('<i class="icon-credit-card"></i>');
        }
    });
});
...
function ps_getBrand(numCartao) {
	var currentBrand = $('#ps_cartao_bandeira').val().toLowerCase();
	if (numCartao.length > 10){
		var card_bin = numCartao.substring(0,6);
		PagSeguroDirectPayment.getBrand({
			...
			success: function(resposta) {
				...
				if (bandeira.toLowerCase() != currentBrand) {
...
function ps_getInstallments(bandeira) {
    ...
    PagSeguroDirectPayment.getInstallments({
        ...
        success: function (resposta) {
            ...
            var opts = DOMPurify.sanitize('<option value=""> -- </option>', {
            ...
            for (var i in parcelamento) {
				...
				if (parcelaMinima == 0) {
	                DOMPurify.sanitize(opts += '<option value="' + optionItem.quantity + '" dataPrice="' + valor + '">' + optionLabel + '</option>', {
	                    SAFE_FOR_JQUERY: true
	                });
				}else if(parcelaMinima >= 1 && tipoParcelaMinima == 0){
		            if (optionItem.quantity <= maxParcelas) {
			            if (parcelaMinima > optionValor) {
							//
						}else{
					        DOMPurify.sanitize(opts += '<option value="' + optionItem.quantity + '" dataPrice="' + valor + '">' + optionLabel + '</option>', {
					            SAFE_FOR_JQUERY: true
				...
				}else if(parcelaMinima >= 1 && tipoParcelaMinima == 1){
		            if (optionItem.quantity <= maxParcelas) {
			            if (parcelaMinima > optionValor) {
...
function ps_validarCartao() {
    ...
    if (typeof errorFields !== 'undefined' && errorFields.length > 0) {
        for (var i = 0; i < errorFields.length; i++) {
            ...
            $('#' + errorFields[i]).parent().parent().addClass('form-error').removeClass('form-ok');
        }
    }
...
function checkField(field_id) {
    if (field_id == 'card_name') {
        var cardName = $('#' + field_id).val().trim();
        if (cardName.length > 3) {
			if (cardName.match('^[a-z A-Z]{3,45}$')) {
				$('#' + field_id).parent().parent().removeClass('form-error').addClass('form-ok');
			} else {
            	$('#' + field_id).parent().parent().addClass('form-error').removeClass('form-ok');
        	}
		} else {
            	$('#' + field_id).parent().parent().addClass('form-error').removeClass('form-ok');
		}
    } else if (field_id == 'card_birth') {
        var cardBirth = $('#' + field_id).val().replace(/[^0-9]/g, '');
        if (cardBirth.length < 8) {
            $('#' + field_id).parent().parent().addClass('form-error').removeClass('form-ok');
        } else {
            $('#' + field_id).parent().parent().removeClass('form-error').addClass('form-ok');
        }
    } else if (field_id == 'card_year' || field_id == 'card_month') {
        var cardYear = $('#' + field_id + ' option:selected').val();
        if (cardYear.length < 2) {
            $('#' + field_id).parent().parent().addClass('form-error').removeClass('form-ok');
        } else {
            $('#' + field_id).parent().parent().removeClass('form-error').addClass('form-ok');
        }
    } else if (field_id == 'card_inst') {
        var cardInst = $('#' + field_id + ' option:selected').val();
        if (cardInst < 1) {
            $('#' + field_id).parent().parent().addClass('form-error').removeClass('form-ok');
        } else {
            $('#' + field_id).parent().parent().removeClass('form-error').addClass('form-ok');
        ...
        var cardPhone = $('#' + field_id).val().replace(/[^0-9]/g, '');
        ...
        var cardCvv = $('#' + field_id).val().replace(/[^0-9]/g, '');
        if (cardCvv.length < 3) {
            $('#' + field_id).parent().parent().addClass('form-error').removeClass('form-ok');
        } else {
            $('#' + field_id).parent().parent().removeClass('form-error').addClass('form-ok');
...
function ps_validarCPF(id) {
    ...
    if (err == 0) {
        cpfField.parent().parent().removeClass('form-error').addClass('form-ok');
        return true;
    } else {
        cpfField.parent().parent().addClass('form-error').removeClass('form-ok');
...
function validarCNPJ(id) {
    ...
    if (cnpj === '') {
        cnpjField.parent().parent().removeClass('form-error').removeClass('form-ok');
    } else if (((dig1 * 10) + dig2) == digito) {
        cnpjField.parent().parent().removeClass('form-error').addClass('form-ok');
        return true;
    } else {
        cnpjField.parent().parent().addClass('form-error').removeClass('form-ok');
...
function valCartao() {
    ...
    if (cardType === "hipercard") {
        cardField.parent().parent().removeClass('form-error').addClass('form-ok');
        ...
        if ((checksum % 10) === 0) {
            ...
            cardField.parent().parent().addClass('form-error').removeClass('form-ok');
            if (msg_console === 1) {
...
function checkCVV(bandeira) {
    ...
    if (cvvField.val()) {
        if (brand == 'amex' && cvvField.val().length != 4 || brand != 'amex' && cvvField.val().length != 3) {
            cvvField.parent().parent().removeClass('form-ok').addClass('form-error');
            if (msg_console === 1) {
...
function validarTel(fieldId) {
    "use strict";
    var foneField = $("#" + fieldId);
    var fone = foneField.val()
        if (!fone || fone === false) {
            fone = $('#card_phone').val();
        }
        if (fone === '') {
            foneField.parent().parent().removeClass('form-ok').removeClass('form-error');
            return;
        }
    ...
    if (reg.test(clean)) {
        ...
        if (dddExists < 0) {
            ...
            foneField.parent().parent().removeClass('form-ok').addClass('form-error');
            ...
            foneField.parent().parent().removeClass('form-error').addClass('form-ok');
            return true;
        }
    } else {
        if (msg_console === 1) {
...
function populateCard(brand) {
    ...
    $("#credit-icon, .card-brand").html('<img class="addon-img" src="' + baseDir + 'modules/pagseguropro/img/' + bandeira.toLowerCase() + '-mini.png" alt="' + bandeira + '" />');
    if (brand === 'aura') {
        $('#card_number').attr('maxlength', 19);
    } else {
        $('#card_number').attr('maxlength', 16);
...
function showError(str, t) {
    ...
    $('html, body').stop().animate({
        scrollTop: $('#pagseguro-container').offset().top
    }, 300);
    setTimeout(function () {
