...
class PagSeguroPro extends PaymentModule {
    ...
    public function hookDisplayAdminOrder()
    {
		...
		if (Tools::isSubmit('cancelarPedidoPagSeguro')) {
			if($transaction->status < 3) {
				if (isset($info['credencial']) && $info['credencial'] != '' && isset($info['token_codigo']) && $info['token_codigo'] != ''){
					$retorno = $this->cancelTransaction($info['cod_transacao'], $info['credencial'], $info['token_codigo']);
				}else{
			...
			if($transaction->status == 3 || $transaction->status == 4 || $transaction->status == 5) {
				...
				if (isset($info['credencial']) && $info['credencial'] != '' && isset($info['token_codigo']) && $info['token_codigo'] != ''){
					if(isset($valor) && (int)$valor > 0){
						$retorno = $this->refundTransaction($info['cod_transacao'], $valor, $info['credencial'], $info['token_codigo']);
					}else{
						$retorno = $this->refundTransaction($info['cod_transacao'], false, $info['credencial'], $info['token_codigo']);
					}
				}else{
					if(isset($valor) && (int)$valor > 0){
	 ...
	 * $code = Código da transação
	 */
    public function cancelTransaction($code, $credential = false, $token_codigo = false)
	{
		$credentials == strtoupper($credential);
		if ($credentials === false || $credentials == '' || $credentials == 'TOKEN') { 
			$order_credentials = array(
		...
		$retorno = $this->curl_send('POST',$this->urls['transaction'].'/cancels?'.$code, $order_credentials, 20, false);
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
        }else{
	 ...
	 */
    public function refundTransaction($code, $value = false, $credential = false, $token_codigo = false) 
	{
		$credentials == strtoupper($credential);
		if ($credentials === false || $credentials == '' || $credentials == 'TOKEN') { 
			$order_credentials = array(
