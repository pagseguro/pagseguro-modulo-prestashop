...
class PagSeguroPro extends PaymentModule {
    ...
    public function __construct()
	{
		...
		/*$this->appId14 = 'prestabr-teste';
		$this->appKey14 = 'B75A3E366464F02BB4215F8A3CEA5DE9';
		$this->appId30 = 'prestabr-teste-30';
		$this->appKey30 = 'D890A2FE5B5BB1D334D46F8F1E489B4D';*/
        ...
        $this->description = $this->l('Utilize o PagSeguro - Checkout Transparente na sua loja');
    ...
    public function hookDisplayPaymentReturn($params) 
	{
        ...
        $transaction = $this->getTransaction($cod_transacao);
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'bool' => false,
                        'desc' => $this->l('Você pode utilizar o Ambiente de Testes (Sandbox)  e testar tudo antes de colocar em Produção.'),
                        'values' => array(
                            array(
			...
			'options' => array(
				...
				'name' => 'name',
                    ...
                    array(
                        ...
                        'desc' => $this->l('Defina o valor do desconto. Exemplo: 15.00'),
                    ),
                    array(
                        'type' => 'switch',
	...
	public function insertPagSeguroData($data)
	{
		if($this->getOrderData($data['cod_transacao'], 'cod_transacao')) {
			...
			$insQuery = 'INSERT INTO `'._DB_PREFIX_.'pagseguropro` (`id_shop`, `cod_cliente`, `cpf_cnpj`, `id_cart`, `id_order`, `referencia`, `cod_transacao`, `buyer_ip`, `status`, `desc_status`, `pagto`, `desc_pagto`, `parcelas`, `url`, `data_pedido`, `data_atu`) VALUES ('.(int)$shop_id.', "'.$data['cod_cliente'].'", "'.$data['cpf_cnpj'].'", '.$data['id_cart'].', '.$data['id_order'].', "'.$data['referencia'].'", "'.$data['cod_transacao'].'", "'.$data['buyer_ip'].'", "'.$data['status'].'", "'.$data['desc_status'].'", "'.$data['pagto'].'", "'.$data['desc_pagto'].'", '.(int)$data['parcelas'].', "'.$data['url'].'", "'.$data['data_pedido'].'", "'.date("Y-m-d H:i:s").'")';
    ...
    public function processarCartao($dados_form) 
	{
		...
		}elseif($tipo_credencial == 'D14' || $tipo_credencial == 'D30'){
	...
	{
		$retorno = $this->curl_send('GET',$this->urls['transaction'].'/'.$code, $this->credentials, 30, false);
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
        }else{
	...
	{
		$retorno = $this->curl_send('POST',$this->urls['transaction'].'/cancels?'.$code, $this->credentials, 20, false);
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
        }else{
	 ...
	 * $code = Código da transação
	 */
    public function refundTransaction($code, $value = false) 
	{
		if($value !== false){
			$retorno = $this->curl_send('POST',$this->urls['transaction'].'/refunds?transactionCode='.$code.'refundValue='.$value, $this->credentials, 20, false);
		}else{
			$retorno = $this->curl_send('POST',$this->urls['transaction'].'/refunds?transactionCode='.$code, $this->credentials, 20, false);
		}
        if ($retorno['error'] == 0) {
            $xml = simplexml_load_string($retorno['resposta']);
            return $xml;
    ...
    private function procDadosCartao($dados_form) 
	{
        ...
        $this->ps_params['installmentValue'] = number_format($dados_form['valor_parcelas_cartao'], 2, '.', '');
    ...
    public function curl_send($method, $post_url, array $data = null, $timeout = 10, $id_cart = false) 
	{
		...
		$header = array(
			...
			'cms-description: prestashop-v.'._PS_VERSION_,
