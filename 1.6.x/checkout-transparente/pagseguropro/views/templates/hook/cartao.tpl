	...
	<fieldset class="pagamento col-xs-12 col-sm-6 pull-left">
		<div class="form-group clearfix row text-left" align="center">
			...
			<div class="input-group col-xs-12">
				<input name="card_name" data-validate="isName" class="form-control" type="text" id="card_name" value="{if (isset($senderName) && $senderName)}{$senderName}{/if}" size="30" onBlur="sendToCard(this.value, 'card-name');checkField('card_name');" required />
			</div>
		</div>
		<div class="form-group clearfix row text-left" align="center">
			<label class="text-left col-xs-12" for="card_doc">{l s='CPF:' mod='pagseguropro'}</label>
			<div class="input-group col-xs-12">
				<input id="card_doc" class="form-control" name="card_doc" {if $device == 'm'}type="tel"{else}type="text"{/if} maxlength="19" onkeyUp="mascara(this,cpfmask);" onFocus="sendToCard($('#card_name').val(), 'card-name');" onblur="verifica('card_doc');$('#boleto_doc, #transf_doc').val(this.value);mascara(this,cpfmask);" value="{if (isset($cpf) && $cpf)}{$cpf}{/if}" size="30" placeholder="{l s='Somente números' mod='pagseguropro'}" required />
			</div>
		</div>
		<div class="form-group clearfix row text-left" align="center">
			<label class="text-left col-xs-12" for="card_doc">{l s='Data de Nascimento:' mod='pagseguropro'}</label>
			<div class="input-group col-xs-12">
				<input id="card_birth" class="form-control" name="card_birth" {if $device == 'm'}type="tel"{else}type="text"{/if} onkeypress="mascara(this,data)" maxlength="10" placeholder="{l s='DD/MM/AAAA' mod='pagseguropro'}" value="{if (isset($birthday) && $birthday)}{$birthday}{/if}" onblur="checkField('card_birth');" required />
			</div>
		</div>
		<div class="form-group clearfix row text-left" align="center">
			<label class="text-left col-xs-12" for="card_phone">{l s='Telefone de contato:' mod='pagseguropro'}</label>
			<div class="input-group col-xs-12">
				<input id="card_phone" class="form-control" name="card_phone" {if $device == 'm'}type="tel"{else}type="text"{/if} maxlength="15" onkeypress="mascara(this,telefone)" onblur="validarTel('card_phone');$('#boleto_phone, #transf_phone').val(this.value);mascara(this,telefone);checkField('card_phone');" value="{if (isset($phone) && $phone)}{$phone}{/if}" placeholder="(99) 99999-9999" required />
			</div>
		</div>
		<div class="form-group clearfix row text-left" align="center">
			<label class="text-left col-xs-12" for="card_number">{l s='Número do cartão:' mod='pagseguropro'}</label>
			<div class="input-group col-xs-12">
				<input id="card_number" class="form-control" name="card_number" {if $device == 'm'}type="tel"{else}type="text"{/if} maxlength="16" min="0" onKeyDown="javascript:if(this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" onFocus="sendToCard($('#card_name').val(), 'card-name');" autocomplete="off" onpaste="return false" value="" onBlur="valCartao();if(this.value.length > 14) sendToCard(this.value, 'card-number');" size="16" required />
		...
		<div class="form-group clearfix row text-left">
			<div class="col-xs-8 nopadding">
				...
				<div class="text-right col-xs-5 nopadding-right">
					...
					<select id="card_month" name="card_month" class="number form-control" onChange="sendToCard(this.value, 'card-expiry-month');checkField(this.id);" required>
						<option value=""> -- </option>
				...
				<div class="text-right col-xs-5 nopadding-left">
					<select id="card_year" name="card_year" class="number form-control" onFocus="sendToCard($('#card_month').val(), 'card-expiry-month');" onChange="sendToCard(this.value.slice(-2), 'card-expiry-year');checkField(this.id);" onBlur="sendToCard(this.value.slice(-2), 'card-expiry-year');" required>
						<option value=""> -- </option>
		...
		<div id="parcelamento" class="form-group clearfix row text-left" align="center">
			...
			<div class="col-xs-12 col-sm-10 col-md-8">
				<select id="card_inst" name="card_inst" class="number form-control" onchange="ps_informarParcela(this.id);" onblur="checkField(this.id);" required>
					<option value="">--</option>
				</select>
			</div>
		</div>
			...
			<div class="panel-body">
				<div class="col-xs-12 col-sm-6 pull-left nopadding">
					<div class="form-group clearfix text-left">
						...
						<div class="input-group col-xs-12">
							<input class="form-control" type="text" name="ps_cartao_cep_cobranca" id="ps_cartao_cep_cobranca" autocomplete="off" maxlength="9" value="{if isset($address_invoice->postcode)}{$address_invoice->postcode}{/if}" required>
						</div>
					</div>
					<div class="form-group clearfix text-left">
						<label for="ps_cartao_endereco_cobranca" class="text-left col-xs-12">{l s='Endereço:' mod='pagseguropro'}</label>
						<div class="input-group col-xs-12">
							<input class="form-control text-left col-xs-12" type="text" name="ps_cartao_endereco_cobranca" id="ps_cartao_endereco_cobranca" autocomplete="off" maxlength="80" value="{if isset($address_invoice->address1)}{$address_invoice->address1}{/if}" required>
						</div>
					</div>
					<div class="form-group clearfix text-left">
						<label for="ps_cartao_numero_cobranca" class="text-left col-xs-12">{l s='Número:' mod='pagseguropro'}</label>
						<div class="input-group col-xs-12">
							<input class="form-control" type="text" name="ps_cartao_numero_cobranca" id="ps_cartao_numero_cobranca" autocomplete="off" maxlength="10" value="{if isset($number_invoice)}{$number_invoice}{/if}" required>
						</div>
					</div>
					<div class="form-group clearfix text-left">
				...
				<div class="col-xs-12 col-sm-6 pull-left nopadding">
					<div class="form-group clearfix text-left">
						...
						<div class="input-group col-xs-12">
							<input class="form-control text-left col-xs-12" type="text" name="ps_cartao_bairro_cobranca" id="ps_cartao_bairro_cobranca" autocomplete="off" maxlength="60" value="{if isset($address_invoice->address2)}{$address_invoice->address2}{/if}" required>
						</div>
					</div>
					<div class="form-group clearfix text-left">
						<label for="ps_cartao_cidade_cobranca" class="text-left col-xs-12">{l s='Cidade:' mod='pagseguropro'}</label>
						<div class="input-group col-xs-12">
							<input class="form-control text-left col-xs-12" type="text" name="ps_cartao_cidade_cobranca" id="ps_cartao_cidade_cobranca" autocomplete="off" maxlength="60" value="{if isset($address_invoice->city)}{$address_invoice->city}{/if}" required>
						</div>
					</div>
					<div class="form-group clearfix text-left">
						<label for="ps_cartao_uf_cobranca" class="text-left col-xs-12">{l s='Estado:' mod='pagseguropro'}</label>
						<div class="input-group col-xs-12">
							<select id="ps_cartao_uf_cobranca" name="ps_cartao_uf_cobranca" class="form-control" required>
								<option value=""> -- </option>
