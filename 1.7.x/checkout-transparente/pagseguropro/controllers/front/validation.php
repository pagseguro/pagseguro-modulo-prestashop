...
class PagSeguroProValidationModuleFrontController extends ModuleFrontController 
{
    ...
    public function postProcess()
    {
		...
		if (!$authorized) {
			die($this->trans('This payment method is not available.', array(), 'Modules.PagSeguroPro.Shop'));
		}

        if (in_array($this->context->currency->iso_code, $this->module->limited_currencies) == false) {
			$this->$pagseguro_msg = $this->trans('Moeda não permitida para esta opção de pagamento.', array(), 'Modules.PagSeguroPro.Shop');
		...
		$this->autorizacao = $this->module->processarCartao($dados_form);
            if (!$this->autorizacao || (string)$this->autorizacao->code == '' || !(string)$this->autorizacao->code) {
				...
				if (Configuration::get('PAGSEGUROPRO_MODO') == 0) {
					Tools::dieObject($this->module->ps_errors);
					Tools::dieObject($this->module->ps_params);
				}else{
					$this->retornoErroApi();
				}
            }
            ...
            if (!$this->autorizacao || (string)$this->autorizacao->code == '' || !(string)$this->autorizacao->code) {
				$code = (int)($this->context->cart->id_customer).'BOLETOPAGSEGURO'.$this->context->cart->id;
				if(CartRule::cartRuleExists($code))
				{
					...
					if(!$CartRule->delete())
					{
						$this->module->saveLog('error', 'Desconto não deletado', $idCartRule, json_encode($this->module->ps_params), json_encode($this->module->ps_errors));
					}
				}
		...
		if (isset($this->autorizacao) && $this->autorizacao) {
			...
			*/
			...
			if ($this->tipo_pagamento == 'cartao') {
				//Tools::dieObject($this->cod_status);
    ...
    private function criaPedidoInicial($transacao)
    {
		...
		$payment_type = $this->autorizacao->paymentMethod->type;

		if($payment_type == 2)
		{
			$payment_option = "Boleto Bancário";
		}else{
			$payment_option = $this->module->parseTipoPagamento($this->autorizacao->paymentMethod->code);
		}

		if (!$this->module->validateOrder($this->cart_id, Configuration::get('_PS_OS_PAGSEGUROPRO_0'), $total_compra, $payment_option, NULL, NULL, $this->context->currency->id, false, $secure_key)) {
		    $this->module->saveLog('error', 'Criar Pedido', $this->cart_id, json_encode($transacao), 'Erro ao criar pedido inicial.');
		}else{
    		$this->id_order = $this->module->currentOrder;
		    $this->pedido = new Order((int)$this->id_order);
			$this->module->updateOrderStatus($this->cart_id, (int)$this->cod_status, (int)$this->id_order);
		}
        if($this->pedido !== false) {
			if ($this->cod_status == 2 || $this->cod_status == 3) {
			    $this->module->updateOrderStatus($this->cart_id, $this->cod_status, (int)$this->id_order);
			}
		    $this->endereco_final = 'index.php?controller=order-confirmation&id_cart='.$this->cart_id.'&id_module='.$this->module->id.'&id_order='.$this->id_order.'&key='.$this->context->cart->secure_key;
    	    $this->redirectFinal();
        }
    }
    ...
    {
		$pagseguro_msg = $this->trans('Pagamento não autorizado. Algum dado pode estar incorreto ou ocorreu algum problema com a operadora. Por favor, revise seus dados de pagamento e tente novamente.', array(), 'Modules.PagSeguroPro.Shop');
    ...
    private function retornoAtualizado($transacao, $wait = false)
    {
        ...
        $tipo_credencial = Configuration::get('PAGSEGUROPRO_CREDENTIAL');
		if ($tipo_credencial == 'TOKEN' || !$tipo_credencial || $tipo_credencial == ''){
			$token_codigo = Configuration::get('PAGSEGUROPRO_TOKEN');
		}elseif($tipo_credencial == 'D14'){
			$token_codigo = Configuration::get('PAGSEGUROPRO_AUTHCODE_D14');
		}elseif($tipo_credencial == 'D30'){
			$token_codigo = Configuration::get('PAGSEGUROPRO_AUTHCODE_D30');
		}
		$transaction = $this->module->getTransaction($this->cod_transacao, $tipo_credencial, $token_codigo, $this->context->cart->id);
		if (!$transaction){
    ...
    {
		$pagseguro_msg = $this->trans('Forma de pagamento temporariamente indisponível. Por favor, tente novamente ou escolha outra forma de pagamento.', array(), 'Modules.PagSeguroPro.Shop');
		$this->context->smarty->assign('pagseguro_msg',$pagseguro_msg);
