...
class PagSeguroProValidationModuleFrontController extends ModuleFrontController 
{
    ...
    private function criaPedidoInicial($transacao)
    {
		...
		if (!$this->module->validateOrder($this->context->cart->id, Configuration::get('_PS_OS_PAGSEGUROPRO_0'), $total_compra, $payment_option, NULL, NULL, $this->context->currency->id, false, $secure_key)) {
    ...
    private function retornoAtualizado($transacao, $wait = false)
    {
		...
		$transaction = $this->module->getTransaction($this->cod_transacao, $tipo_credencial, $token_codigo);
		if (!$transaction){
