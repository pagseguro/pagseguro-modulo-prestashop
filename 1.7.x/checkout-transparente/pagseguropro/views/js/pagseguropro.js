...
$(document).ready(function() {
	...
	//Check TOS
	$('#conditions-to-approve .custom-checkbox input[type=checkbox]').change(function() {
		var selectedPaymentOption = $('.payment-options input[name=payment-option]:checked').attr('data-module-name');

        if(this.checked) {
			if (selectedPaymentOption == 'pagseguropro'){
				if ($('#checkout_pagseguropro').is(":visible")){
					checkTokenGeneration(false);
				}else if($('#boleto_pagseguropro').is(":visible")){
					ps_validarBoleto();
				}else if($('#debito_pagseguropro').is(":visible")){
					ps_validarTransf(false);
				}
			}
        }else{
	...
	$('.payment-options input[name="payment-option"]').change(function() {
		setTimeout(function(){ 
			var selectedPaymentOption = $('.payment-options input[name=payment-option]:checked').attr('data-module-name');
	...
	$('#debito_pagseguropro input[name="ps_transf"]').change(function() {
		ps_validarTransf(false);
	});
...
function checkTokenGeneration(create){
	if (valCartao() !== false
...
function ps_finalizarBoleto() {
    if (ps_validarBoleto(true) !== false) {
		ps_setSenderHash();
		var hashComprador = PagSeguroDirectPayment.getSenderHash();
...
function ps_validarCartao(show) {
    var html = '';
    ...
    if (typeof errorFields !== 'undefined' && errorFields.length > 0 && show !== false) {
...
function ps_validarBoleto(show) {
    var html = '';
...
function ps_validarTransf(show) {
    var html = '';
    ...
    if (telefone.length == 0) {
    	html += 'Telefone não preenchido. <br>';
    }else if (!validarTel('transf_phone')) {
		html += 'Telefone inválido. <br>';
	}
...
function valCartao() {
	...
	if (creditCardNumber.length > 10){
		...
		if (creditCardNumber.length > 0 && msg_console === 1) {
			console.log('cartão incompleto!');
		}
		return false;
	}
}
...
function checkTos(valid){
	if(valid === true){
		//validar TOS
		if (!$('#conditions-to-approve .custom-checkbox input[type=checkbox]').prop('required') || $('#conditions-to-approve .custom-checkbox input[type=checkbox]').is(":checked")) {
			setTimeout(function(){ 
...
function validarTel(fieldId) {
	...
	if (!foneField || typeof(foneField) == 'undefined') {
		foneField = $('#card_phone');
	}
...
function checkField(field_id) {
	...
	if (valid !== false){
		$('#'+field_id).parent().parent().removeClass('has-danger').addClass('has-success');
		$('#'+field_id).addClass('form-control-success').removeClass('form-control-danger');
	}else{
		$('#'+field_id).parent().parent().addClass('has-danger').removeClass('has-success');
		$('#'+field_id).removeClass('form-control-success').addClass('form-control-danger');
	}
}
