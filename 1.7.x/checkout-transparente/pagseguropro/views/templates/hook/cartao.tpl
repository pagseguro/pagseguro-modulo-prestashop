...
<div class="container-credito checkout_pagseguro clearfix">
	...
	<form id="checkout_pagseguropro" name="checkout" action="{$link->getModuleLink('pagseguropro', 'validation', [], true)|escape:'html'}" method="post" class="clearfix" onsubmit="return ps_finalizarCartao();">
		...
		<input type="hidden" name="parcelasSemJuros" id="parcelasSemJuros" value="{$parcelasSemJuros}" />
		...
		<fieldset class="pagamento col-lg-6 col-md-12 col-sm-12 col-xs-12 pull-left nopadding-left">
			...
			<div class="form-group clearfix row" align="center">
				<label class="text-left col-xs-12" for="card_doc">{l s='CPF/CNPJ:' d='Modules.PagSeguroPro.Shop'}</label>
				<div class="input-group col-xs-12">
					<input id="card_doc" class="form-control" name="card_doc" {if $device == 'm'}type="tel"{else}type="text"{/if} maxlength="19" onFocus="sendToCard($('#card_name').val(), 'card-name');" onblur="verifica('card_doc');$('#boleto_doc, #transf_doc').val(this.value);" value="{if (isset($cpf) && $cpf)}{$cpf}{/if}" size="30" placeholder="{l s='Somente números' d='Modules.PagSeguroPro.Shop'}" />
				</div>
			</div>
			<div class="form-group clearfix row" align="center">
				<label class="text-left col-xs-12" for="card_doc">{l s='Data de Nascimento:' d='Modules.PagSeguroPro.Shop'}</label>
				<div class="input-group col-xs-12">
					<input id="card_birth" class="form-control" name="card_birth" type="date" onkeypress="" value="{if (isset($birthday) && $birthday)}{$birthday}{/if}" />
				</div>
			</div>
			<div class="form-group clearfix row" align="center">
