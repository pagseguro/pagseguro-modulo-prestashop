	...
	public function postProcess()
	{
		...
		if (isset($this->pag_response->error_messages[0]) && $this->pag_response->error_messages[0] ||
			...
			if ($this->pag_response->charges[0]->payment_method->type == 'BOLETO') {
				$message_p = "Não foi possível autorizar a emissão do boleto. Por favor, tente novamente utilizando outro meio de pagamento.";
			} else {
				$message_p = $this->pag_response->charges[0]->payment_response->message;
			}
			$pagbank_msg = 'Pagamento não autorizado.<br />Motivo: '.$message_p.'<br />(Código: '.$this->pag_response->charges[0]->payment_response->code.')';
			if (_PS_VERSION_ >= '1.7.0') {
				$this->redirect_link = 'index.php?controller=order&step=3';
			} else {
				if(Configuration::get('PS_ORDER_PROCESS_TYPE') == 1 && Dispatcher::getInstance()->getController() != 'orderopc'){
