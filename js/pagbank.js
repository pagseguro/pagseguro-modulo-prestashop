...
function addGooglePayButton() {
	var paymentsClient = getGooglePaymentsClient();
	var show_btn_google = paymentsClient.createButton({onClick: onGooglePaymentButtonClicked});
...
function infoAndBrandGooglePayment(paymentBrand, paymentLastDigits, signature) {
	...
	document.getElementById('google_signature').value = signature;
		...
		success: function (data, xhr) {
			...
			if (response.error_messages && response.error_messages != false || Array.isArray(response.error_messages)) {
				...
				installments.forEach((parc) => {
					...
					} else if (optionQty == 1 && discount_type > 0 && google_pay_value > 0 && discount_google == 1) {
