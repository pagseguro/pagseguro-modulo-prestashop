		...
		success: function (data, xhr) {
			...
			if (response.errors && response.errors != false) {
				...
				installments.forEach((parc, index) => {
					...
					if (optionQty == 1 && discount_type > 0 && order_discount_value > 0 && discount_card == 1) {
						discountCalculated = (optionValue  - order_discount_value);
						optionValue = discountCalculated;
					}
...
function updateActiveOption(option) {
	if (option) { //Verificar
		document.getElementById('opc_payment_methods').scrollTop;
	}
}
...
function populateSelect(brand) {
    ...
    document.getElementById('card_association').value = brand;
}
...
function showLoading(hide, id) {
	...
	if (!hide || hide == ''){
		...
		if (ps_version < '1.7') {
			...
			document.getElementById('submitBankSlip').disabled = true;
			document.getElementById('submitPix').disabled = true;
		}
		...
		if (ps_version < '1.7') {
			...
			document.getElementById('submitBankSlip').disabled = false;
			document.getElementById('submitPix').disabled = false;
		}
