...
class PagBank extends PaymentModule
{
	...
	*/
	...
	public function hookPaymentOptions($params)
	{
		...
		if ($active_payments['google_pay'] && strlen(Configuration::get('PAGBANK_GOOGLE_MERCHANT_ID')) >= 13 ) {
	...
	*/
	...
	{
		$gpay_id = strtoupper(md5((int)$this->context->cart->id));
	...
	public function processCardPayment($form_data)
	{
		...
		if ($this->ps_params['paymentType'] == 'credit_card') {
			...
			if ((int)$saved_card > 0) {
				$check_saved = $this->getCustomerToken($id_customer, (int)$form_data['card_token_id']);
			...
			"card": {
				...
				"wallet": {
					"type": "GOOGLE_PAY",
					"key": ' . json_encode(str_replace("&quot;", "", $this->ps_params['googleSignature'])) . '
				}
			}';
		}
