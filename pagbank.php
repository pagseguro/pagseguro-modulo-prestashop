...
class PagBank extends PaymentModule
{
	...
	*/
	...
	public function hookDisplayPaymentReturn($params)
	{
		...
		$this->smarty->assign(array(
			...
			'ps_order' => $order,
			'ps_paid_state' => Configuration::get('PAGBANK_PAID'),
			'this_path' => Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ . 'modules/' . $this->name . '/',
			'url_update' => Tools::getShopDomainSsl(true, true) . __PS_BASE_URI__ . 'modules/' . $this->name . '/update.php',
		));
	...
	*/
	...
	public function insertPagBankData($data)
	{
		if ($this->getOrderData($data['transaction_code'], 'transaction_code')) {
			...
			$ins_query .= ' VALUES (' . (int)$shop_id . ', "' . (int)$data['id_customer'] . '", "' . pSQL($data['cpf_cnpj']) . '", ' . (int)$data['id_cart'] . ', ' . (int)$data['id_order'] . ', "' . pSQL($data['reference']) . '", "' . pSQL($data['transaction_code']) . '", "' . pSQL($data['buyer_ip']) . '", "' . pSQL($data['status']) . '", "' . pSQL($data['status_description']) . '", "' . pSQL($data['payment_type']) . '", "' . pSQL($data['payment_description']) . '", ' . (int)$data['installments'] . ', "' . pSQL($data['nsu']) . '", "' . pSQL($data['url']) . '", "' . pSQL($this->credential_type) . '", "' .(int)$capture. '", "' .(int)$data['environment']. '", "' . pSQL($data['date_add']) . '", "' . date("Y-m-d H:i:s") . '")';
	...
	*/
	...
	public function captureTransaction($code, $value, $order)
	{
		...
		if (!$api_response['errors']) {
			if ($this->updateOrderStatus($order->id_cart, 'PAID', $order->id, date("Y-m-d H:i:s"))) {
				if (Db::getInstance()->execute(
					'UPDATE `' . _DB_PREFIX_ . 'pagbank` set `status`="PAID", `status_description`="' . $this->parseStatus('PAID') . '", `capture`="1" WHERE `id_cart`="' . $order->id_cart . '"'
				)) {
					return true;
				} else {
	...
	*/
	...
	public function saveLog($type, $method, $id_cart = false, $data = false, $response = false, $url = false, $cron = 0)
	{
		...
		$query .= ' (NOW(), "' . pSQL($type) . '", "' . pSQL($method) . '", ' . $id_cart . ', "' . addslashes($clear_data) . '", "' . addslashes($clear_response) . '" , "' . pSQL(addslashes($url)) . '", ' . $cron . ')';
	...
	public function getAppAuthorization($code, $app)
	{
		...
		if ((int)Configuration::get('PAGBANK_FULL_LOG') == 1) {
			if (isset($resp_object->error_messages)) {
				$this->saveLog('error', 'POST', '', $json_authorization, $resp, $post_url);
			} else {
				$this->saveLog('sucess', 'POST', '', $json_authorization, $resp, $post_url);
			}
		}
		...
		if (isset($resp_object->error_messages)) {
			...
			if ((int)$this->environment == 1){
				if ($app == 'TAX') {
					Configuration::updateValue('PAGBANK_TOKEN_TAX', $resp_object->access_token, false);
					Configuration::updateValue('PAGBANK_TOKEN_EXPIRES_TAX', $expire_date, false);
					Configuration::updateValue('PAGBANK_TOKEN_REFRESH_TAX', $resp_object->refresh_token, false);
				} elseif ($app == 'D14') {
					Configuration::updateValue('PAGBANK_TOKEN_D14', $resp_object->access_token, false);
					Configuration::updateValue('PAGBANK_TOKEN_EXPIRES_D14', $expire_date, false);
					Configuration::updateValue('PAGBANK_TOKEN_REFRESH_D14', $resp_object->refresh_token, false);
				} elseif ($app == 'D30') {
					Configuration::updateValue('PAGBANK_TOKEN_D30', $resp_object->access_token, false);
					Configuration::updateValue('PAGBANK_TOKEN_EXPIRES_D30', $expire_date, false);
					Configuration::updateValue('PAGBANK_TOKEN_REFRESH_D30', $resp_object->refresh_token, false);
				}
			} else {
				if ($app == 'TAX') {
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_TAX', $resp_object->access_token, false);
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_EXPIRES_TAX', $expire_date, false);
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_REFRESH_TAX', $resp_object->refresh_token, false);
				} elseif ($app == 'D14') {
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_D14', $resp_object->access_token, false);
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_EXPIRES_D14', $expire_date, false);
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_REFRESH_D14', $resp_object->refresh_token, false);
				} elseif ($app == 'D30') {
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_D30', $resp_object->access_token, false);
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_EXPIRES_D30', $expire_date, false);
					Configuration::updateValue('PAGBANK_TOKEN_SANDBOX_REFRESH_D30', $resp_object->refresh_token, false);
				}
			}
	...
	public function refreshToken($refreshToken, $app)
	{
		...
		if ((int)Configuration::get('PAGBANK_FULL_LOG') == 1) {
			if (isset($resp_object->error_messages)) {
				$this->saveLog('error', 'POST', '', $json_refresh, $resp, $post_url);
			} else {
				$this->saveLog('sucess', 'POST', '', $json_refresh, $resp, $post_url);
			}
		}
	...
	*/
	public function insertCustomerToken($info)
	{
		...
		$ins_query .= ' VALUES (' . (int)$info['id_customer'] . ', "' . pSQL($info['card_name']) . '", "' . pSQL($info['card_brand']) . '", ' . (int)$info['card_first_digits'] . ', ' . (int)$info['card_last_digits'] . ', "' . pSQL($info['card_month']) . '", ' . (int)$info['card_year'] . ', "' . pSQL($info['card_token']) . '", "' . date("Y-m-d H:i:s") . '")';
