...
class PagBank extends PaymentModule
{
	...
	public function __construct()
	{
		...
		if (!$this->token || $this->token == '' || !$this->public_key || $this->public_key == '') {
			$this->ready = false;
		} else {
			$this->ready = true;
		}
	...
	public function install()
	{
		...
		Configuration::updateValue('PAGBANK_CREDENTIAL', '', false); //D14, D30 TAX
		...
		Configuration::updateValue('PAGBANK_TOKEN_SANDBOX', '', false);
		...
		Configuration::updateValue('PAGBANK_SHOW_CONSOLE', 1, false);
	...
	public function getContent()
	{
		...
		$app_info_tax = $this->getApiInfo((int)$this->environment, 'TAX');
		$app_info_d14 = $this->getApiInfo((int)$this->environment, 'D14');
		$app_info_d30 = $this->getApiInfo((int)$this->environment, 'D30');
		...
		$this->context->smarty->assign(array(
			...
			'valid_token' => $this->token,
			'new_user_code' => 'prestabr_code_' . $this->environment . '_' . $this->generateRandomKey(),
			...
			'client_id' => array('tax' => $app_info_tax['client_id'], 'd14' => $app_info_d14['client_id'], 'd30' => $app_info_d30['client_id']),
			'signin_connect_url' => $this->environment == 1 ? 'https://connect.pagseguro.uol.com.br/oauth2/authorize' : 'https://connect.sandbox.pagseguro.uol.com.br/oauth2/authorize',
			...
			'link_transactions' => $this->context->link->getAdminLink("AdminPagBank", false) . '&token=' . Tools::getAdminTokenLite("AdminPagBank"),
			'link_logs' => $this->context->link->getAdminLink("AdminPagBankLogs", false) . '&token=' . Tools::getAdminTokenLite("AdminPagBankLogs"),
			'link_page' => $this->context->link->getAdminLink("AdminModules", false) . '&token=' . Tools::getAdminTokenLite("AdminModules") . '&configure=pagbank&tab_module=payments_gateways&module_name=pagbank',
	...
	public function hookDisplayBackOfficeHeader()
	{
		...
		if (!empty($token_expires_tax) && !empty($token_refresh_tax) && (strtotime($today) > strtotime($token_expires_tax))) {
			$this->context->controller->warnings[] = 'Atualizar Token PagBank - TAX';
		...
		if (!empty($token_expires_d14) && !empty($token_refresh_d14) && (strtotime($today) > strtotime($token_expires_d14))) {
			$this->context->controller->warnings[] = 'Atualizar Token PagBank - D14';
		...
		if (!empty($token_expires_d30) && !empty($token_refresh_d30) && (strtotime($today) > strtotime($token_expires_d30))) {
			$this->context->controller->warnings[] = 'Atualizar Token PagBank - D30';
	...
	public function paymentLogic($params)
	{
		...
		if ($discounts['discount_type'] > 0 && $discounts['discount_value'] > 0) {
			if ($discounts['discount_type'] == 1) {
				$discount_value = number_format(($total_products * ($discounts['discount_value'] / 100)), 2, '.', '');
			} else {
		...
		$this->smarty->assign(array(
			...
			'pix_timeout' => (int)$pix_expiration,
	...
	public function hookDisplayPaymentReturn($params)
	{
		...
		if (isset($transaction->qr_codes)) {
			...
			$payment_type = 'PIX';
			$deadline_pix = Configuration::get('PAGBANK_PIX_TIME_LIMIT');
			$pix['id'] = $qr->id;
			$pix['text'] = $qr->text;
			if ($deadline_pix > 60) {
				$array_time = $this->hoursMinutes($deadline_pix);
				$pix['deadline'] = array(
	...
	public function hookDisplayAdminOrder()
	{
		...
		$payment_description = isset($transaction->charges) ? $info['payment_description'] : 'PIX - PagBank';
		...
		if (Tools::isSubmit('refundOrderPagBank')) {
			if (in_array($status_pagbank, array('AUTHORIZED', 'PAID', 'AVAILABLE', 'DISPUTE'))) {
				$value = Tools::getValue('refundValue');
		...
		$this->context->smarty->assign(array(
			...
			'status_pagbank' => $this->parseStatus($status_pagbank),
	...
	*/
	...
	public function deleteStatus()
	{
		Db::getInstance()->execute("
			UPDATE `" . _DB_PREFIX_ . "order_state` SET `deleted` = 1 WHERE `module_name` = '" . $this->name . "'
		");
		return true;
	}
	...
	public function insertPagBankData($data)
	{
		if ($this->getOrderData($data['transaction_code'], 'transaction_code')) {
			...
			if ($this->credential_type == 'TAX' || !$this->credential_type || $this->credential_type == '') {
				...
				$token_code = Configuration::get('PAGBANK_TOKEN_D14');
			} elseif ($this->credential_type == 'D30') {
				$token_code = Configuration::get('PAGBANK_TOKEN_D30');
			}

			$ins_query = 'INSERT INTO `' . _DB_PREFIX_ . 'pagbank` (`id_shop`, `id_customer`, `cpf_cnpj`, `id_cart`, `id_order`, `reference`, `transaction_code`, `buyer_ip`, `status`, `status_description`, `payment_type`, `payment_description`, `installments`, `url`, `credential`, `token_code`, `date_add`, `date_upd`)';
			$ins_query .= ' VALUES (' . (int)$shop_id . ', "' . $data['id_customer'] . '", "' . $data['cpf_cnpj'] . '", ' . $data['id_cart'] . ', ' . $data['id_order'] . ', "' . $data['reference'] . '", "' . $data['transaction_code'] . '", "' . $data['buyer_ip'] . '", "' . $data['status'] . '", "' . $data['status_description'] . '", "' . $data['payment_type'] . '", "' . $data['payment_description'] . '", ' . (int)$data['installments'] . ', "' . $data['url'] . '", "' . $this->credential_type . '", "' . $token_code . '", "' . $data['date_add'] . '", "' . date("Y-m-d H:i:s") . '")';
	...
	*/
	public function processPixPayment($form_data)
	{
		...
		if (
			(int)Configuration::get('PAGBANK_DISCOUNT_TYPE') > 0 &&
			(int)Configuration::get('PAGBANK_DISCOUNT_VALUE') > 0 &&
			(int)Configuration::get('PAGBANK_DISCOUNT_PIX') == 1
		) {
			$discount_value = (float)Configuration::get('PAGBANK_DISCOUNT_VALUE');
			$discount_type = Configuration::get('PAGBANK_DISCOUNT_TYPE');
			if ($discount_type == 1) {
				$total_paid = ((float)$total_paid * (1 - ($discount_value / 100)));
			} else {
				$total_paid = ((float)$total_paid - $discount_value);
			}
		}
		...
		$json_pix = '{
			...
			"qr_codes": [
				{
					"amount": {
						"value": ' . (float)number_format($total_paid, 2, "", "") . '
					},
	...
	public function processCardPayment($form_data)
	{
		...
		if (
			(int)Configuration::get('PAGBANK_DISCOUNT_TYPE') > 0 &&
			(int)Configuration::get('PAGBANK_DISCOUNT_VALUE') > 0 &&
			(int)Configuration::get('PAGBANK_DISCOUNT_CREDIT') == 1 &&
			(int)$this->ps_params['installmentQuantity'] == 1
		) {
			$discount_value = (float)Configuration::get('PAGBANK_DISCOUNT_VALUE');
			$discount_type = Configuration::get('PAGBANK_DISCOUNT_TYPE');
			if ($discount_type == 1) {
		...
		$json_credit_card = '{
			...
			"charges": [
				{
					"reference_id": "' . $this->ps_params['reference'] . '",
					"description": "Pedido realizado na loja ' . Configuration::get('PS_SHOP_NAME') . ', em ' . date("d/m/Y") . ', no valor total de R$ ' . number_format($total_paid, 2, ",", ".") . '",
					"amount": {
						"value": ' . (float)number_format($total_paid, 2, "", "") . ',
						"currency": "BRL"
					},
	...
	public function processBankSlipPayment($form_data)
	{
		...
		if (
			(int)Configuration::get('PAGBANK_DISCOUNT_TYPE') > 0 &&
			(int)Configuration::get('PAGBANK_DISCOUNT_VALUE') > 0 &&
			(int)Configuration::get('PAGBANK_DISCOUNT_BANKSLIP') == 1
		) {
			$discount_value = (float)Configuration::get('PAGBANK_DISCOUNT_VALUE');
			$discount_type = Configuration::get('PAGBANK_DISCOUNT_TYPE');
			if ($discount_type == 1) {
	 ...
	 */
	public function refundTransaction($code, $value, $id_cart = false)
	{
		$json_refund = '{
		  "amount": {
			"value": ' . $value . '
		...
		$api_response = $this->curl_send('POST', $this->urls['api'] . 'charges/' . $code . '/cancel', $json_refund, 20, $id_cart);
		$api_response_status = $api_response['response']->status;
		if (!$api_response['errors']) {
			if (Db::getInstance()->execute(
				'UPDATE `' . _DB_PREFIX_ . 'pagbank` set `refund`="' . ($value / 100) . '", `status`="' . $api_response_status . '", `status_description`="' . $this->parseStatus($api_response_status) . '" WHERE `id_cart`="' . $id_cart . '"'
			)) {
				return $api_response;
			} else {
	...
	public function updateOrderStatus($id_cart, $status_code, $id_order = null, $date_update = null)
	{
		...
		$status_ps = $this->correspondStatus($status_code);
	...
	private function processBasicData()
	{
		...
		$this->ps_params['reference'] = $this->context->cart->id . '.' . Configuration::get('PAGBANK_CREDENTIAL') . '.' . $this->generateRandomKey();
	...
	*/
	...
	/*
	 * Processa valores extra (descontos e embalagem para presente) para envio ao PagBank
	*/
	private function processExtraValues()
	{
		$addition = $this->context->cart->getOrderTotal(true, Cart::ONLY_WRAPPING);
		$discount = $this->context->cart->getOrderTotal(true, Cart::ONLY_DISCOUNTS);

		if ($discount > 0) {
			$discount *= -1;
		}
		$extra_value = $addition + $discount;
		if ($extra_value != 0) {
			$this->ps_params['extraAmount'] = number_format($extra_value, 2, '.', '');
		}
	}
	...
	*/
	private function processFormData($form_data)
	{
		...
		if ($this->environment == 0) {
			...
			$email = $email_arr[0] . '@sandbox.pagseguro.com.br';
		}
		if (strlen($email) > 60) {
		...
		$this->ps_params['sender_name'] = $nome;
		...
		$cidade = $address_invoice->city;
		if (strlen($cidade) > 60) {
			$cidade = substr($cidade, 0, 60);
		}
		$cep = preg_replace('/[^0-9]/', '', $address_invoice->postcode);
		$state = new State((int)$address_invoice->id_state);
		$uf_nome = $state->name;
		$uf_sigla = $state->iso_code;
		$this->ps_params['billingAddressPostalCode'] = isset($form_data['invoice_postcode']) && $form_data['invoice_postcode'] != '' ? preg_replace('/[^0-9]/', '', $form_data['invoice_postcode']) : $cep;
		...
		$this->ps_params['billingAddressCity'] = isset($form_data['invoice_city']) && $form_data['invoice_city'] != '' ? $form_data['invoice_city'] : $cidade;
		$this->ps_params['billingAddressState'] = isset($form_data['invoice_state']) && $form_data['invoice_state'] != '' ? $this->getStateByIsoCode($form_data['invoice_state'], 'name') : $uf_nome;
	...
	{
		$cpf_cnpj = preg_replace('/[^0-9]/', '', $form_data['cpf_cnpj']);
		$titular_cartao = trim($form_data['card_name']);
		$titular_cartao = preg_replace('/\s(?=\s)/', '', $titular_cartao);
		$this->ps_params['creditCardHolderName'] = $titular_cartao;
		$telephone = $this->formatPhoneNumber($form_data['telephone']);
	...
	*/
	public function curl_send($method, $post_url, $json_data = false, $timeout = 10, $id_cart = false)
	{
		...
		if (isset($id_cart)) {
			$info = $this->getOrderData($id_cart, 'id_cart');
			if (!empty($info) && $info['token_code'] != '') {
				$token_api = $info['token_code'];
			}
		}
		$curl = curl_init();
		if ($json_data !== false && (strtoupper($method) === 'POST' || strtoupper($method) === 'PUT')) {
			$post_fields = ($json_data ? $json_data : '');
			$content_length = 'Content-length: ' . strlen($post_fields);
		...
		if (Configuration::get('PAGBANK_FULL_LOG') !== false) {
	...
	*/
	...
	public function getAppAuthorization($code, $app)
	{
		...
		$post_url = $this->environment == 1 ? 'https://api.pagseguro.com/oauth2/token' : 'https://sandbox.api.pagseguro.com/oauth2/token';
		$header = array(
	...
	public function refreshToken($refreshToken, $app)
	{
		...
		$post_url = $this->environment == 1 ? 'https://api.pagseguro.com/oauth2/refresh' : 'https://sandbox.api.pagseguro.com/oauth2/refresh';
		$header = array(
			'Authorization: Pub ' . $criptogram,
			'Content-Type: application/json',
		);
