...
class PagBank extends PaymentModule
{
	...
	*/
	public function calculateDiscounts($payment_type, $installment = false)
	{
		...
		if (
			in_array($payment_type, $discount_options) &&
			...
			if($payment_type == 'credit_card' && $installment == 1 || $payment_type != 'credit_card') {
				if ($discount_type == 1) {
					$total_partial = ($total_products * (1 - ($discount_value / 100)));
					$total_paid = round(($total_partial + $total_shipping), 2, PHP_ROUND_HALF_DOWN);
				} else {
					$total_partial = ($total_products - $discount_value);
					$total_paid = ($total_partial + $total_shipping);
				}
			}
		} else {
	...
	*/
	...
	public function processCardPayment($form_data)
	{
		...
		$total_paid = $this->calculateDiscounts('credit_card', (int)$this->ps_params['installmentQuantity']);
