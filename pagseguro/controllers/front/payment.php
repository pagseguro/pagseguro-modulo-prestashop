...
function exectPayment()
{
    /* SSL Management */
    $useSSL = true;
    $pagseguro = new PagSeguro();
    setVariablesPaymentExecutionView($pagseguro->context);
...
function setVariablesPaymentExecutionView($context)
{
    global $smarty;
    ...
    if ($context->cart->id_currency != $id_currency && ! is_null($id_currency)) {
        ...
        $smarty->assign(array(
            'total_real' => convertPriceFull($context->cart->getOrderTotal(true, Cart::BOTH), new Currency($context->cart->id_currency), new Currency($id_currency)),
    ...
    $smarty->assign(array(
        ...
        'action_url' => _PS_VERSION_ < '1.5' ? _PS_BASE_URL_ . __PS_BASE_URI__ . 'modules/pagseguro/controllers/front/validation.php"' : _PS_BASE_URL_ . __PS_BASE_URI__ . 'index.php?fc=module&module=pagseguro&controller=validation'
    ));
...
class PagSeguroPaymentModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        parent::initContent();
        setVariablesPaymentExecutionView($this->context);
