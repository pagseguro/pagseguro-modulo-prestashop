...
class PagSeguro extends PaymentModule
{
    ...
    private function getConciliationTabHtml()
    {
        ...
        $this->context->smarty->assign('regError', $tableResult['regError']);
        $conteudo = "";
    ...
    /*
     * Checks URL availability through cURL
    */
    private function cUrl() {
    	$curl = new StdClass();
    	$curl->ws        = curl_init("https://ws.pagseguro.uol.com.br");
    	$curl->stc       = curl_init("https://stc.pagseguro.uol.com.br");
    	$curl->pagseguro = curl_init("https://pagseguro.uol.com.br");
    ...
    private function displayCurlErr($err) {
    
    	$this->context->smarty->assign('version', $this->_whichVersion());
    	$this->context->smarty->assign('module_dir', _PS_MODULE_DIR_ . 'pagseguro/');
    	$this->context->smarty->assign('css_version', $this->getCssDisplay());
    	$this->context->smarty->assign('err', $err[0]);
    
    	$this->html .= $this->display(__PS_BASE_URI__ . 'modules/pagseguro', 'views/templates/front/curl.tpl');
    
    	return $this->html;
    
    }
    
    private function cUrlErr($errno)
    {
    	$error_codes=array(
