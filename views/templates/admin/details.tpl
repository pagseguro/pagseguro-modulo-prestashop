	...
	<div class="panel card panel-info">
		...
		<div class="panel-body card-body nopadding">
			<div class="row">
				<div class="col-xs-12 col-sm-6">
					<ul class="list list-unstyled">
						...
						{/if}
						<li><b>{l s='Status:' mod='pagbank'}</b> <span>{$status_pagbank}</span></li>
						<li><b>{l s='Forma de Pagamento:' mod='pagbank'}</b> <span>{$payment_method}</span></li>
						{if isset($transaction->charges) && isset($transaction->charges[0]->payment_method->installments)}
							<li><b>{l s='Quantidade de parcelas:' mod='pagbank'}</b>
								<span>{$transaction->charges[0]->payment_method->installments}</span></li>
						{/if}
						{if isset($transaction->qr_codes[0])}
							<li><b>{l s='Link do PIX:' mod='pagbank'}</b> <span>{$transaction->qr_codes[0]->text}</span>
							</li>
						{/if}
			...
			<div class="panel-footer">
				{if (in_array($status_pagbank, ['AUTHORIZED', 'WAITING', 'WAITING_PIX', 'IN_ANALYSIS']))}
					<form action="{$this_page|escape:'htmlall':'UTF-8'}" method="post" class="form-horizontal well"
						style="float: left;">
						<button name="cancelarPedidoPagBank" type="submit" class="btn btn-danger btn-lg"
							onclick="return confirm('{l s='Tem certeza que deseja cancelar a transação no PagBank?' mod='pagbank' js=1}');">
							{l s='Cancelar Transação no PagBank' mod='pagbank'}
						</button>
					</form>
				{elseif (in_array($status_pagbank, ['PAID', 'AVAILABLE', 'DISPUTE']))}
					<form action="{$this_page|escape:'htmlall':'UTF-8'}" method="post"
						class="form-horizontal form-inline well" style="float: left;">
						<div class="alert alert-danger">
							{* <p> {l s='O valor total deste pedido no PagBank é:' mod='pagbank'} <b>R${$transaction->grossAmount}</b></p> *}
							<p> {l s='Se por algum motivo for necessário estornar um valor parcial ou total, informe o valor no campo abaixo:' mod='pagbank'}
							</p>
						</div>
						<div class="form-group">
							...
							<div class="input-group col-xs-6">
								<input type="text" name="refundValue" class="input form-control" value=""
									onkeypress="mascara(this,valormask)" placeholder="{$order->total_paid}" />
							</div>
						</div>
			...
			<pre class="transaction hidden">{$transaction|print_r}</pre>
			<pre class="info hidden">{$info|print_r}</pre>
		</div>
	</div>
	<div class="clearfix">
