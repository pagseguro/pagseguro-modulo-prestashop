...
<div class="row" id="pagbank">
	...
	{if $info['refund'] > 0}
		<div class="alert alert-info">
			...
			<p>{l s='Para mais detalhes acesse a sua conta no PagBank, no menu, "Extratos e Relatórios > Extrato de Transações".' mod='pagbank'}</p>
					...
					<ul class="list list-unstyled">
						...
						{if isset($transaction->charges) && isset($transaction->charges[0]->payment_method->installments)}
							<li>
								...
								{if $info['capture'] == 0 && isset($transaction->charges[0]->amount->summary->paid) && $transaction->charges[0]->amount->summary->paid > 0}<br /><b>{l s='Total Capturado:' mod='pagbank'}</b> <span>{displayPrice price=($transaction->charges[0]->amount->summary->paid/100) currency=$order->id_currency}</span>{/if}
							</li>
						{/if}
