			...
			<div class="col-xs-12 col-sm-8 col-lg-6 data_waiting" id="pay_links">
				...
				{elseif ($payment_type === 'PIX')}
					<div class="panel panel-success" id="pix_window">
						...
						<div class="panel-body">
							<p class="text-center">
								<img src="{$pix.link}" alt="{$pix.text}" class="img-responsive" style="margin:auto; max-width:220px;" />
								<br />
								<textarea id="pix_text" class="form-control" rows="5" onClick="this.select();">{$pix.text}</textarea>
								<br /><br />
								<button id="pix_text_button" class="btn btn-info border" data-clipboard-target="#pix_text" data-clipboard-action="copy">
									{l s='Copiar código Pix' mod='pagbank'}
								</button>
							</p>
							<p class="alert alert-warning text-center">
								{if $alternate_time}
									...
									<br />
								{else}
									{l s='Efetue o PIX imediatamente.' mod='pagbank'}<br />
									{l s='O pedido tem um prazo de' mod='pagbank'}
									<span class="prazo_pix">
										{if {$pix.deadline.hours} > 0}
											{$pix.deadline.hours} {l s='horas.' mod='pagbank'}
										{/if}
										{if {$pix.deadline.minutes} > 0}
											{$pix.deadline.minutes} {l s='minutos.' mod='pagbank'}
										{/if}
									</span>
								{/if}
								<br />
								{l s='Você deve efetuar o pagamento até:' mod='pagbank'}
								<br />
				...
				{elseif ($payment_type === 'WALLET')}
					<div class="panel panel-success" id="wallet_window">
						...
						<div class="panel-body">
							<p class="text-center">
								{if $device === 'd'}
									<img src="{$wallet.link}" alt="{$wallet.text}" class="img-responsive" style="margin:auto; max-width:220px;" />
									<br />
									<textarea id="wallet_text" class="form-control" rows="4" onClick="this.select();">{$wallet.text}</textarea>
									<br /><br />
									<button id="wallet_text_button" class="btn btn-info border" data-clipboard-target="#wallet_text" data-clipboard-action="copy">
										{l s='Copiar código' mod='pagbank'}
									</button>
								{else}
							...
							<p class="alert alert-warning text-center">
								{l s='Efetue o pagamento imediatamente.' mod='pagbank'}<br />
								{l s='O pedido tem um prazo de' mod='pagbank'}
								<span>
									{if {$wallet.prazo.hours} > 0}
										{$wallet.prazo.hours} {l s='horas.' mod='pagbank'}
									{/if}
									{if {$wallet.prazo.minutes} > 0} 
										{$wallet.prazo.minutes} {l s='minutos.' mod='pagbank'}{/if}
								</span>
								<br />
								{l s='Você deve efetuar o pagamento até:' mod='pagbank'}
								<br />
	...
	{if ($payment_type === 'PIX')}
		<div id="pix_success" class="clearfix">
			<div id="proccess_pix" style="display:none;" class="container clearfix">
				...
				<div class="col-xs-12 col-sm-12 text-center" id="pagbank_msg">
					{l s='PIX Recebido!' mod='pagbank'}<br />{l s='Redirecionando...' mod='pagbank'}
				</div>
			</div>
		</div>
	...
	{if ($payment_type === 'WALLET')}
		<div id="wallet_success" class="clearfix">
			<div id="proccess_wallet" style="display:none;" class="container clearfix">
				...
				<div class="col-xs-12 col-sm-12 text-center" id="pagbank_msg">
					{l s='Pagamento Recebido!' mod='pagbank'}<br />{l s='Redirecionando...' mod='pagbank'}
				</div>
			</div>
		</div>
