						...
						<ul class="list list-unstyled">
							...
							{if isset($transaction->charges) && isset($transaction->charges[0]->payment_method->installments)}
								<li>
									...
									<b>{if isset($transaction->charges[0]->amount->fees) && $transaction->charges[0]->amount->fees}{l s='Total c/ juros:' d='Modules.PagBank.Admin'}{else}{l s='Total s/ juros:' d='Modules.PagBank.Admin'}{/if}</b> <span>{displayPrice price=($transaction->charges[0]->amount->value/100) currency=$order->id_currency}</span>
									{if $info['capture'] != null && $info['capture'] == 0 && isset($transaction->charges[0]->amount->summary->paid) && $transaction->charges[0]->amount->summary->paid > 0}<br /><b>{l s='Total Capturado:' d='Modules.PagBank.Admin'}</b> <span>{displayPrice price=($transaction->charges[0]->amount->summary->paid/100) currency=$order->id_currency}</span>{/if}
								</li>
							{/if}
