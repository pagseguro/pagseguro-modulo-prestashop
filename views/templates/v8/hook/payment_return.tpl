...
<div id="pagbank-confirmation" class="container">
	...
	<pre class="hidden">{$payment_type}</pre>
				...
				<div class="card col-xs-12 col-sm-12 col-lg-12 data_waiting nopadding">
					...
					<div class="card-body">
						...
						<ul class="list clearfix">
							...
							<li><b>Valor do pedido:</b> {Tools::displayPrice($ps_order_value)}</li>
					...
					<tfoot>
						<tr class="total_prods">
							...
							<td colspan="2" class="price text-xs-right">
								{Tools::displayPrice($ps_order->total_products|escape:'htmlall':'UTF-8')}</td>
						</tr>
						{if ($ps_order->total_discounts) > 0}
							<tr class="extra">
								<td colspan="3">{l s='Descontos' d='Modules.PagBank.Shop'}</td>
								<td colspan="2" class="price text-xs-right">
									{Tools::displayPrice($ps_order->total_discounts|escape:'htmlall':'UTF-8')}</td>
							</tr>
						{/if}
						{if ($ps_order->total_wrapping) > 0}
							<tr class="extra">
								...
								<td colspan="2" class="price text-xs-right">
									{Tools::displayPrice($ps_order->total_wrapping|escape:'htmlall':'UTF-8')}</td>
							</tr>
						{/if}
						<tr class="frete">
							<td colspan="3">{l s='Frete' d='Modules.PagBank.Shop'}</td>
							<td colspan="2" class="price text-xs-right">
								{Tools::displayPrice($ps_order->total_shipping|escape:'htmlall':'UTF-8')}</td>
						</tr>
						<tr class="total">
							<td colspan="3">{l s='Total do Pedido' d='Modules.PagBank.Shop'}</td>
							<td colspan="2" class="price text-xs-right">
								{Tools::displayPrice($ps_order->total_paid|escape:'htmlall':'UTF-8')}</td>
						</tr>
					</tfoot>
				</table>
			</div>
		...
		<div id="pix_success" class="form-group clearfix row" align="center">
			<div id="proccess_pix" style="display:none;" class="container clearfix">
				<div class="row">
					...
					<div class="hidden-xs hidden-xs-down col-sm-3 nopadding-left" id="pagbank_logo" align="center">
						<img src="{$this_path}img/pagbank-logo-animado_35px.gif" class="img-responsive" />
					</div>
					<div class="hidden-xl-down hidden-lg hidden-lg-down hidden-md hidden-md-down hidden-sm hidden-sm-down col-xs-3 nopadding-left"
						id="pagbank_logo" align="center">
